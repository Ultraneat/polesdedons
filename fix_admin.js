...
// Insert heroPanel1 after logoPanel1
let idx1 = code.indexOf('id="logo-settings-panel-overview"');
if (idx1 !== -1) {
    let saveIdx = code.indexOf('Enregistrer le Logo', idx1);
    let endDiv = code.indexOf('              </div>', saveIdx) + 20; // after </div>
    code = code.substring(0, endDiv) + '\n' + heroPanel1 + code.substring(endDiv);
}

// Insert heroPanel2 after logoPanel2
let idx2 = code.indexOf('id="logo-settings-panel"');
if (idx2 !== -1) {
    let saveIdx = code.indexOf('Enregistrer le Logo', idx2);
    let endDiv = code.indexOf('              </div>', saveIdx) + 20; // after </div>
    code = code.substring(0, endDiv) + '\n' + heroPanel2 + code.substring(endDiv);
}
